...
void draw_right_animation(lv_obj_t *canvas, int *current_src_index) {
    ...
    if (current_src_index == 0) {
        lv_animimg_set_src(art, (const void **)crystal_anim_imgs, right_anim_img_counts[current_src_index]);
    }
    else if (current_src_index == 1) {
        lv_animimg_set_src(art, (const void **)onepunch_anim_imgs, right_anim_img_counts[current_src_index]);
    }
